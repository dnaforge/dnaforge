...
export function getXuon(graph: Graph){
    const eToSubE = new Map<MEdge, Edge>();
    for(let e1 of graph.edges){
        const [v1, v2] = e1.getVertices();
        for(let e2 of v1.getAdjacentEdges()){
            if(e1 == e2) continue;
        }
...
function getMstar(startEdge: MEdge){
    ...
    while(stack.length > 0){
        ...
        if(exhausted.has(edge)) continue;

        st.add(edge);
        exhausted.add(edge);
        for(const nE of edge.neighbours){
            if(visited.has(nE)) exhausted.add(nE);
            else visited.add(nE);
            stack.push(nE);
        }
    }
...
function blossom(e0: MEdge, e1: MEdge, b: MEdge){
    ...
    getGs(gs0, e0, b0, t0);
    getGs(gs1, e1, b1, t1);
    gs0.sort((a: MEdge, b:MEdge) => {return a.serial - b.serial});
    gs1.sort((a: MEdge, b:MEdge) => {return a.serial - b.serial});
...
function matroidParity(edges: MEdge[]){
    ...
    while(queue.length > 0){
        const e = queue.shift();
        const sortedNeighbours =  Array.from(e.DPGNeighbours).sort((a: MEdge, b: MEdge) => {return a.serial - b.serial});
