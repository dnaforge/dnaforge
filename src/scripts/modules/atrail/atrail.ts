...
class ATrail extends WiresModel {
    ...
    initialiseGraph() {
        if (!this.graph.hasFaceInformation()) throw `Graph has insufficient face-information for topological routing.`;
        try {
        ...
        const getEuler = () => {
            ...
            while (visited.size < nEdges) {
                ...
                for (let i = 0; i < atrail.length; i++) {
                    ...
                    const t = new Set(atrail[((i - 1) + atrail.length) % atrail.length].getVertices());
    ...
    setATrail(trail: Array<number>) {
        ...
        for (let i = 1; i < trail.length; i++) {
            ...
            if (edge == null) throw `No such edge: ${[trail[i - 1] + 1, trail[i] + 1]}`;
            if (visited.has(edge)) {
                if (edge.twin) {
                    if (visited.has(edge.twin)) throw `Trying to traverse an edge more than twice.`;
                    else edge = edge.twin;
                }
...
function wiresToCylinders(atrail: ATrail, params: Parameters) {
    ...
    for (let i = 0; i < atrail.trail.length; i++) {
        ...
        if (edge.twin) {
            ...
            const nor = edge.normal.clone().multiplyScalar((-1) ** (visited.has(edge.twin) ? 1 : 0));
    ...
    for (let i = 0; i < cm.cylinders.length; i++) {
        ...
        const other = i == cm.cylinders.length - 1 ? cm.cylinders[0] : cm.cylinders[i + 1];
        const vStackT = vStack.get(c.v2);
        const prev = vStackT[(vStackT.indexOf(other) - 1 + vStackT.length) % vStackT.length];
...
function cylindersToNucleotides(cm: CylinderModel, params: Parameters) {
    ...
    const nm = NucleotideModel.compileFromGenericCylinderModel(cm, minLinkers, maxLinkers, true);
