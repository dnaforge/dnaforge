...
export default abstract class ModuleMenu extends Menu {
    ...
    abstract graphToWires(graph: Graph, params:{[name: string]: number | boolean | string}): WiresModel;

    abstract wiresToCylinders(wires: WiresModel, params: {[name: string]: number | boolean | string}): CylinderModel;

    abstract cylindersToNucleotides(cm: CylinderModel, params: {[name: string]: number | boolean | string}): NucleotideModel;

    activate() {
        try { this.regenerateVisible(); }
    ...
    generateNucleotideModel() {
        ...
        this.context.addMessage(`Created: ${this.nm.length()} nucleotides.`, "info");
    }
    ...
    relaxCylinders() {
        ...
        this.context.addMessage(`Cylinders relaxed.<br>Initial score: ${initialScore}<br>Final score: ${finalScore}`, "info");
    }
    ...
    setupEventListeners() {
        ...
        this.generateNucleotidesButton = $(`#${this.elementId}-generate-nucleotides`);

        const blur = () => {(<HTMLElement>document.activeElement).blur()};
