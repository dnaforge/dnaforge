import Sterna, { graphToWires, wiresToCylinders, cylindersToNucleotides } from './sterna';
...
export default class SternaMenu extends ModuleMenu {
    ...
    graphToWires(graph: Graph, params: {[name: string]: number | boolean | string}) {
        const wires = graphToWires(graph, params);
        this.context.addMessage(`Generated a route around the spanning tree with ${wires.trail.length} edges.`, "info");
        return wires;
    }

    wiresToCylinders(wires: WiresModel, params: {[name: string]: number | boolean | string}) {
        return wiresToCylinders(<Sterna>wires, params);
    }

    cylindersToNucleotides(cm: CylinderModel, params: {[name: string]: number | boolean | string}) {
    ...
    setupEventListeners() {
        ...
        $("#sterna-primary-dialog-confirm").on("click", () => {
            try {
                this.uploadPrimary($("#sterna-primary-dialog-text").val().toUpperCase());
            } catch (error) {
                this.context.addMessage(error, "alert");
                throw (error);
            }
