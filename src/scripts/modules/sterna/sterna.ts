...
function wiresToCylinders(sterna: Sterna, params: {[name: string]: number | boolean | string}) {
    ...
    for (let i = 0; i < trail.length; i++) {
        ...
        const next = i == trail.length - 1 ? edgeToCyl.get(trail[0]) : edgeToCyl.get(trail[i + 1]);
        if (i == trail.length - 1) visited.delete(edgeToCyl.get(trail[0])); // Connect to the first 5' of the first cylinder

        if (visited.has(cur) && !cur.isPseudo || !visited.has(cur) && cur.isPseudo) {
            if (visited.has(next)) {
