import { RNA_PSEUDOKNOTS, RNA_NP_TEMPLATE, IUPAC_RNA } from "../../globals/consts";
...
function generatePartial(nm: NucleotideModel) {
    ...
    for (let i = 0; i < nucleotides.length; i++) if (pseudos[i]) ps[i] = pseudos[i];
...
function generateRandom(nm: NucleotideModel, gcContent: number) {
    ...
    for(let i = 0; i < nucleotides.length; i++){
        ...
        if(pairs.get(i) == i || !visited.has(n.pair)){
            ...
            if(!base) throw `Impossible base pair. ${i}: ${n.base} - ${pairs.get(i)}: ${ps[pairs.get(i)]}`;
        }
        visited.add(n);
        ps.push(base);
    }
