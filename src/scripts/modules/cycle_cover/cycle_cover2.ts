...
export default class CycleCover extends WiresModel {
    ...
    getCycleCover() {
        ...
        const neighbours = new Map<Vertex, Array<Edge>>();
        for(let v of graph.getVertices()){
            try {   var vNeighbours = v.getTopoAdjacentEdges();} 
            catch { var vNeighbours = this.getNeighbours(v);}
            neighbours.set(v, vNeighbours);
            for(let i = 0; i < vNeighbours.length; i++){
                const e1 = vNeighbours[i];
                const e2 = vNeighbours[(i + 1) % vNeighbours.length];
            }
        }
        ...
        const traverse = (e: Edge, v: Vertex) => {
            ...
            const cycle = [e];
            while(curV != other){
                const vNeighbours = neighbours.get(curV);
    ...
    getNeighbours(v: Vertex): Array<Edge> {
        ...
        for (let e1 of neighbours) {
            ...
            distances.set(e1, distsT.sort((a, b) => {
                return a[1] - b[1];
            }));
        }
        // traverse to NN
