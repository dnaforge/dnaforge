...
import { XtdnaParameters } from './xtdna_menu';
...
export class Xtdna extends WiresModel {
  graph: Graph;
  st: Set<Edge>;
  trail: HalfEdge[];
 ...
 * @returns
 */
export function graphToWires(graph: Graph, params: XtdnaParameters) {
  const xtdna = new Xtdna(graph);
  return xtdna;
}
 ...
 * @returns
 */
export function wiresToCylinders(xtdna: Xtdna, params: XtdnaParameters) {
  const scale = params.scale;
  const cm = new CylinderModel(scale, params.naType);

  const trail = xtdna.trail;
  const st = xtdna.st;
