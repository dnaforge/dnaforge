...
class Cylinder {
    ...
    constructor(startP: Vector3, dir: Vector3, length: number, scale = 1, naType = "DNA") {
        this.id = UID++;
        this.scale = scale;
        this.naType = naType;
        ...
        this.dir = dir;                                                                     
        this.nor1 = r1.sub(this.dir.clone().multiplyScalar(r1.dot(this.dir))).normalize();  
        ...
        this.p2 = this.p1.clone().add(this.dir.clone().multiplyScalar(((this.length - 1) * this.nucParams.RISE + this.nucParams.INCLINATION) * this.scale));
    }
    ...
    getPrimePosition(str: string) : Vector3{
        ...
        const rise = this.dir.clone().multiplyScalar((this.length - 1) * this.nucParams.RISE * this.scale);
        const twist = (this.length - 1) * this.nucParams.TWIST;
        switch (str) {
            case "first5Prime":
                const nor5P1 = this.nor1.clone();
                return p1.add(nor5P1.multiplyScalar(this.scale * this.nucParams.RADIUS_BB_CENTER));

            case "first3Prime":
                const nor3P1 = this.nor1.clone().applyAxisAngle(this.dir, twist);
                return p1.add(rise).add(nor3P1.multiplyScalar(this.scale * this.nucParams.RADIUS_BB_CENTER));

            case "second5Prime":
                const nor5P2 = this.nor1.clone().applyAxisAngle(this.dir, twist + this.nucParams.AXIS);
                return p2.add(nor5P2.multiplyScalar(this.scale * this.nucParams.RADIUS_BB_CENTER));

            case "second3Prime":
                const nor3P2 = this.nor1.clone().applyAxisAngle(this.dir, this.nucParams.AXIS);
                return p2.sub(rise).add(nor3P2.multiplyScalar(this.scale * this.nucParams.RADIUS_BB_CENTER));
    ...
    getStrand2Coords() : [number, Vector3, Vector3, Vector3]{
        ...
        const startNormal = this.nor1.clone().multiplyScalar(-1).applyAxisAngle(this.dir, (N - 1) * this.nucParams.TWIST + this.nucParams.AXIS);
    ...
    setObjectInstance(index: number, meshes: Record<string, THREE.InstancedMesh>) {
        const transform = new Matrix4().makeBasis(this.nor2, this.dir, this.nor1).scale(new Vector3(this.scale, this.scale, this.scale));

        const center = this.p1.clone().add(this.p2).multiplyScalar(0.5);
        let cylLength = this.p2.clone().sub(this.p1).length() / this.scale;
        if (this.nucParams.INCLINATION < 0) cylLength -= 2 * this.nucParams.INCLINATION;

        const transformMain = transform.clone().setPosition(center).scale(new Vector3(1, cylLength, 1));
        ...
        for (let i = 0; i < 4; i++) {
            ...
            if (p1 && p2) {
                ...
                transformLinker = get2PointTransform(p1, p2).scale(new Vector3(this.scale, length, this.scale));
            }
            else {
...
class CylinderModel {
    ...
    generateMesh() {
        ...
        const cylinderMain = new THREE.CylinderGeometry(this.nucParams.RADIUS, this.nucParams.RADIUS, 1, 8);
        const cylinderTips = new THREE.DodecahedronGeometry(0.4, 0);
        const linker = new THREE.CylinderGeometry(0.1, 0.1, 1, 8);

        const meshMain = new THREE.InstancedMesh(cylinderMain, cylindersMaterial, this.cylinders.length);
        const meshPrime = new THREE.InstancedMesh(cylinderTips, cylindersMaterial, 4 * this.cylinders.length);
        const meshLinker = new THREE.InstancedMesh(linker, cylindersMaterial, 4 * this.cylinders.length);
    ...
    async relax(iterations = 1000) {
        ...
        for (let i = 0; i < iterations; i++) {
            ...
            let jitter = 0.05 * Math.PI * (Math.random() - 0.5) * (1 - Math.min(1, i / iterations));
    ...
    calculateTorques() {
        ...
        for (let cyl of this.cylinders) {
            ...
            for (let str of _.keys(cyl.neighbours)) {
                ...
                const relP1 = rel1.clone().sub(dir.clone().multiplyScalar(rel1.dot(dir)));
                const relP2 = rel2.clone().sub(dir.clone().multiplyScalar(rel2.dot(dir)));

                const exp = 1.5
                const alpha = relP1.angleTo(relP2) ** exp * (-1) ** (posPrime.cross(relP2).dot(dir) < 0 ? 1 : 0);
