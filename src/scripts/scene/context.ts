...
export default class Context {
    ...
    focusCamera(point: Vector3) {
        ...
        const d = this.cameraControls.target.clone().sub(this.camera.position).length();
        const dir = this.cameraControls.target.clone().sub(this.camera.position).normalize();
        const nPos = this.cameraControls.target.clone().sub(dir.multiplyScalar(Math.min(d, 3)));
    ...
    flipCameraView() {
        ...
        this.cameraControls.target.copy(new Vector3(-tPos.x, -(tPos.y - 5) + 5, -tPos.z));
        this.cameraControls.update();
    }
    ...
    setCamera(cam: THREE.Camera) {
        ...
        this.cameraControls = new OrbitControls(this.camera, document.querySelector('#canvas'));
        this.cameraControls.update();
    }
    ...
    resetCamera(toOrthographic: boolean) {
        if (toOrthographic) {
            this.camera = new THREE.OrthographicCamera(cameraParams.left, cameraParams.right, cameraParams.top, cameraParams.bottom, cameraParams.near, cameraParams.far);
        }
        else {
            this.camera = new THREE.PerspectiveCamera(cameraParams.fov, cameraParams.aspect, cameraParams.near, cameraParams.far);
        }
