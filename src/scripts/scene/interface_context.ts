...
const meshMaterial = new THREE.MeshBasicMaterial({ color: 0x9999ff, transparent: true, opacity: 0.1 });
...
export default class InterfaceMenu extends Menu {
    ...
    populateHotkeys() {
        ...
        this.hotkeys.set("np1", () => { this.context.setCameraView("front") });
        this.hotkeys.set("np2", () => { this.context.rotateCameraView("down") });
        this.hotkeys.set("np3", () => { this.context.setCameraView("right") });
        this.hotkeys.set("np4", () => { this.context.rotateCameraView("left") });
        this.hotkeys.set("np5", () => { this.switchCameraType()});
        this.hotkeys.set("np6", () => { this.context.rotateCameraView("right") });
        this.hotkeys.set("np7", () => { this.context.setCameraView("top") });
        this.hotkeys.set("np8", () => { this.context.rotateCameraView("up") });
        this.hotkeys.set("np9", () => {this.context.flipCameraView()});
        this.hotkeys.set("np,", () => {this.resetCamera()});
    }
    ...
    addAxes() {
        if (!this.axes) {
            ...
            const Y = new THREE.ArrowHelper(new THREE.Vector3(0, 1, 0), origin, length, 0x000000);
    ...
    */
    ...
    generateWires() {
        ...
        const materialEdge = new THREE.LineBasicMaterial({ color: wireframeColour });
        const materialSplitEdge = new THREE.LineBasicMaterial({ color: wireframeColour2 });
    ...
    generateMesh() {
        ...
        for (let f of graph.getFaces()) {
            ...
            for (let i = 0; i < edges.length; i++) {
                ...
                if ((v2.coords.clone().sub(v1.coords)).cross((v3.coords.clone().sub(v1.coords))).dot(f.normal) > 0) {
                    vertices.push(...<any>v1.coords, ...<any>v2.coords, ...<any>v3.coords);
                }
                else {
                    vertices.push(...<any>v1.coords, ...<any>v3.coords, ...<any>v2.coords);
                }
                normals.push(...<any>f.normal, ...<any>f.normal, ...<any>f.normal);
            }
        }
...
*/
    ...
    resetCamera() {
        const val = (this.context.getCamera() as OrthographicCamera).isOrthographicCamera;
        this.context.resetCamera(val);
    }

    switchCameraType() {
        if ((this.context.getCamera() as OrthographicCamera).isOrthographicCamera) this.setPerspectiveCamera();
    ...
    setupEventListeners() {
        ...
        $("#selection-mode").on("click", () => {
            GLOBALS.selectionMode = $("#selection-mode").children(".active").attr("data-id");
        });
