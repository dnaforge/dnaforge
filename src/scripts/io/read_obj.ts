...
export class OBJLoader extends Loader {
	...
	load(url: string, onLoad: (g: Graph) => void, onProgress: (e: Event) => void, onError: (e: Event) => void) {
		const scope = this;
	...
	parse(text: string) {
		...
		for (let i = 0, l = lines.length; i < l; i++) {
			...
			if (lineFirstChar === 'v') {
				...
				edges.push(
					faceVertices[0],
					faceVertices[faceVertices.length - 1],
				);
		...
		for (let i = 0; i < vertices.length; i++) {
			const v = graph.addVertex(vertices[i].sub(com).multiplyScalar(scale), normals[i]);
			gVertices.push(v);
		}
		//implicit edges:
